...
export default class DatePickerField extends Component {
  ...
  validateDateString = dateString => {
    const parsedDate = moment(dateString, this.props.dateInputFormat);
    if (
    ...
    if (
      ...
      return this.props.maxDateError.replace(
        "{date}",
        moment(this.props.maxDate).format(this.props.dateInputFormat)
      );
    }
    if (
      parsedDate.format(DATE_FORMAT_FOR_MATCHING) <
      ...
      return this.props.minDateError.replace(
        "{date}",
        moment(this.props.minDate).format(this.props.dateInputFormat)
      );
    }
    return "";
  };
  ...
  changeHandler = (event, value) => {
    if (this.isEmpty(value)) {
      ...
      if (errors) {
        ...
        this.props.onChange(
          event,
          moment(value, this.props.dateInputFormat).format(
            this.props.dateValueFormat
          )
        );
      }
    }
  ...
  datePickerHandler = (event, value) => {
    ...
    this.props.onChange(
      event,
      moment(value).format(this.props.dateValueFormat)
    );
  };

  getDateString = date => moment(date).format(this.props.dateInputFormat);
  ...
  getDateValue = value => {
    ...
    const parsedDate = moment(value, this.props.dateValueFormat);
    if (!parsedDate.isValid()) {
      const dateInput = moment(this.state.valueText, this.props.dateInputFormat);
      if (dateInput > moment(this.props.maxDate)) {
        return new Date(this.props.maxDate);
      }
      if (dateInput < moment(this.props.minDate)) {
  ...
  setDatePickerReference = node => {
    if (node) {
      const dialog = node.refs.dialogWindow;
      const hadleWindowKeyUp = dialog.handleWindowKeyUp;
      dialog.handleWindowKeyUp = (...args) => this.state.isDialogOpen ? hadleWindowKeyUp(...args) : () => {};
      this.dialog = dialog;
    }
  };

  openDatePicker = (e) => {
    const isSpacePressed = e && (e.charCode === 0);
    if (isSpacePressed) {
      e.preventDefault();
    }
    if (this.state.isDialogOpen && !isSpacePressed) {
      this.dialog.show();
    }
  };
      ...
      <div className="datepicker-wrap">
        <div className="datepicker-wrap__text-field">
          <TextField
            ...
            hintText={this.props.dateInputFormat}
          ...
          <IconButton
            ...
            onClick={(e) => this.openDatePicker(e)}
        ...
        <div className="datepicker-wrap__datepicker">
          <DatePicker
            ...
            formatDate={date => moment(date).format(this.props.dateInputFormat)}
            ...
            ref={this.setDatePickerReference}
